...
func CreateTransactionByUTXOs(utxoTx *utxo.UTXOTx, acc *account_ron.AccountInfo, amount, tips uint64, fromAccount, toAccount string) (transaction.Transaction, error) {
	//从服务器返回的utxo集合里找到满足转账所需金额的utxo
	tx_utxos, err := getUTXOsWithAmountByUTXOs(
		utxoTx,
		common.NewAmount(amount),
		common.NewAmount(tips),
		common.NewAmount(0),
		common.NewAmount(0))
	if err != nil {
